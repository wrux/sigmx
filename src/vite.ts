...
import { builtinPlugins, customPluginMeta, generatePluginsModule, selectPlugins, type PluginMeta, type Selection } from './kernel/scan.js'
...
export const DEFAULT_EXTENSIONS = ['.astro', '.html', '.mdx', '.md', '.ts', '.tsx', '.js', '.jsx', '.svelte', '.vue', '.php', '.erb', '.twig', '.hbs']
...
const customMeta = (root: string, custom: Record<string, string> = {}): PluginMeta[] =>
  Object.entries(custom).flatMap(([exp, path]) => {
    ...
    if (!meta) console.warn(`[sigmx] could not read plugin metadata for "${exp}" in ${path}; include it via "always" if it must ship`)
    return meta ? [meta] : []
  })
