...
for (const l of libs) {
  ...
  console.log(
    l.name.padEnd(10),
    l.version.padEnd(8),
    `raw=${buf.length} gzip=${out.at(-1).gzip} brotli=${out.at(-1).brotli}`,
  );
}
const file = process.argv[2];
if (file)
  writeFileSync(file, JSON.stringify({ measuredAt: new Date().toISOString().slice(0, 10), libraries: out }, null, 2));
