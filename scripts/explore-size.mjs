...
const R = 'kernel/runtime.ts',
  S = 'kernel/state.ts',
  C = 'kernel/compile.ts';
const variants = {
  'A  no error isolation (errors throw; no onError)': (d) =>
    patch(d, R, [
      [
  ...
  'C  no eventPrefix stripping in core': (d) =>
    patch(d, R, [
      [
  ...
  'D  no namespace enumeration ($ ownKeys/has/delete)': (d) =>
    patch(d, S, [
      [
  ...
  'E  no empty-namespace preservation in snapshot': (d) =>
    patch(d, S, [
      [
  ...
  'F  arrays are plain (no mutation notify)': (d) =>
    patch(d, S, [
  ...
  'G  no last-statement value fallback in expressions': (d) =>
    patch(d, C, [
      [
  ...
  'H  no parsed-attribute cache': (d) =>
    patch(d, R, [
      ['  let p = parsed.get(raw)\n  if (!p) {', '  let p: Parsed | undefined\n  if (!p) {'],
      ['    parsed.set(raw, p)\n', ''],
    ]),
  'I  target esnext instead of es2022': 'esnext',
  'K  kernel cuts B+D+E+G combined': (d) => {
    for (const k of [
      'B  no data-ignore support',
      'D  no namespace enumeration ($ ownKeys/has/delete)',
      'E  no empty-namespace preservation in snapshot',
      'G  no last-statement value fallback in expressions',
    ])
      variants[k](d);
  },
  'J  no late use() re-apply': (d) =>
    patch(d, R, [['    if (added.size) for (const r of roots) apply(r, false, added)\n', '']]),
  'L  request client without form encoding': (d) =>
    patch(d, 'plugins/functions/request.ts', [
      [
        / {4}} else \{\n {6}const form = [\s\S]*?\n {4}}\n {4}return \{ url: u\.toString\(\)/.exec(
          readFileSync(join(d, 'plugins/functions/request.ts'), 'utf8'),
        )[0],
        '    }\n    return { url: u.toString()',
      ],
    ]),
  'M  request client without retry/backoff/reconnect': (d) =>
    patch(d, 'plugins/functions/request.ts', [
      [
  ...
  'N  request client without hidden-tab pausing': (d) =>
    patch(d, 'plugins/functions/request.ts', [
      [
  ...
  'O  morph without pantry / cross-ancestor moves': (d) =>
    patch(d, 'plugins/server-events/morph.ts', [
      [
  ...
  'P  patch-elements without full-document responses': (d) =>
    patch(d, 'plugins/server-events/apply-elements.ts', [
...
const scratch = () => {
  const d = mkdtempSync(join(tmpdir(), 'sigmx-'));
  cpSync('src', join(d, 'src'), { recursive: true });
  writeFileSync(join(d, 'package.json'), '{"sideEffects":false}');
  cpSync('tsconfig.json', join(d, 'tsconfig.json'));
  return join(d, 'src');
};
const base = scratch();
...
console.log(
  ...
  'fetch'.padStart(7),
  'all'.padStart(6),
  '  (delta bytes gzip; negative = smaller)',
);
  ...
  try {
    ...
    console.log(
      ...
      ...Object.keys(cases).map((k) =>
        String(m[k] - baseline[k]).padStart(k === 'minimal' ? 8 : k === 'fetch' ? 7 : 6),
      ),
    );
  } catch (e) {
