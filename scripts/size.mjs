...
const build = async (contents) => {
  const r = await esbuild.build({
    ...
    bundle: true, minify: true, format: 'esm', target: 'es2022', write: false, mangleProps: /^_/, logLevel: 'silent',
  })
...
const plugins = ['signals','computed','effect','ref','text','show','className','style','attr','bind','init','on','onInterval','onIntersect','onSignalPatch','jsonSignals','indicator','onResize','onRaf','matchMedia','scrollIntoView','customValidity','persist','queryString','replaceUrl','viewTransition','animate','peek','setAll','toggleAll','fit','clipboard','intl','httpGet','httpPost','httpPut','httpPatch','httpDelete','applyElements','applyState','boost','cloak','collapse','html','mask','removeMe','teleport','transition','trap','confirm','dispatch','websocket']
const attrNames = { className: 'class', jsonSignals: 'json-signals', onInterval: 'on-interval', onIntersect: 'on-intersect', onSignalPatch: 'on-signal-patch', onResize: 'on-resize', onRaf: 'on-raf', matchMedia: 'match-media', scrollIntoView: 'scroll-into-view', customValidity: 'custom-validity', queryString: 'query-string', replaceUrl: 'replace-url', viewTransition: 'view-transition', httpGet: '@get', httpPost: '@post', httpPut: '@put', httpPatch: '@patch', httpDelete: '@delete', applyElements: 'patch-elements (morph)', applyState: 'patch-signals', peek: '@peek', setAll: '@setAll', toggleAll: '@toggleAll', fit: '@fit', clipboard: '@clipboard', intl: '@intl', removeMe: 'remove-me', confirm: '@confirm', dispatch: '@dispatch', websocket: '@ws' }

const json = process.argv.includes('--json')
const bundles = []
for (const [name, src] of Object.entries(cases)) bundles.push({ name, ...(await build(src)) })
// The shipped standalone file is additionally passed through terser by the build; report it when present.
try {
  const { readFileSync } = await import('node:fs')
  const out = readFileSync('dist/sigmx.standalone.js')
  bundles.push({ name: 'everything, shipped dist/sigmx.standalone.js (esbuild + terser)', raw: out.length, gzip: gzipSync(out, { level: 9 }).length, brotli: brotliCompressSync(out, { params: { [constants.BROTLI_PARAM_QUALITY]: 11 } }).length })
} catch {}
