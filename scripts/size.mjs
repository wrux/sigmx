...
const build = async (contents) => {
  ...
  return {
    ...
    brotli: brotliCompressSync(out, { params: { [constants.BROTLI_PARAM_QUALITY]: 11 } }).length,
  };
};
const withPlugins = (names, extra = '') =>
  `import { createSigmx } from './kernel'; import { ${names.join(', ')} } from './plugins'; ${extra} createSigmx({ plugins: [${names.join(', ')}] })`;
const cases = {
  ...
  'minimal + fetch + server handlers (morph)': withPlugins([
    'signals',
    'text',
    'show',
    'on',
...
const attrNames = {
  className: 'class',
  jsonSignals: 'json-signals',
  onInterval: 'on-interval',
  onIntersect: 'on-intersect',
  onSignalPatch: 'on-signal-patch',
  onResize: 'on-resize',
  onRaf: 'on-raf',
  ...
  httpPatch: '@patch',
  httpDelete: '@delete',
  applyElements: 'patch-elements (morph)',
  applyState: 'patch-signals',
  peek: '@peek',
  setAll: '@setAll',
  toggleAll: '@toggleAll',
  fit: '@fit',
...
try {
  ...
  bundles.push({
    name: 'everything, shipped dist/sigmx.standalone.js (esbuild + terser)',
    raw: out.length,
    gzip: gzipSync(out, { level: 9 }).length,
    brotli: brotliCompressSync(out, { params: { [constants.BROTLI_PARAM_QUALITY]: 11 } }).length,
  });
...
if (json || process.argv.includes('--plugins')) {
  for (const p of plugins) {
    ...
    perPlugin.push({
      ...
      name: attrNames[p] ?? p,
      raw: s.raw - core.raw,
      gzip: s.gzip - core.gzip,
      brotli: s.brotli - core.brotli,
    });
  }
}
if (json) {
  ...
  for (const b of bundles)
    console.log(b.name.padEnd(46), String(b.raw).padStart(6), String(b.gzip).padStart(6), String(b.brotli).padStart(6));
  if (perPlugin.length) {
    console.log('\nplugin (delta over core)'.padEnd(46), '   raw   gzip brotli');
    for (const p of perPlugin.sort((a, b) => b.gzip - a.gzip))
      console.log(
        p.name.padEnd(46),
        String(p.raw).padStart(6),
        String(p.gzip).padStart(6),
        String(p.brotli).padStart(6),
      );
  }
}
