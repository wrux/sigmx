...
const validate = (step: number, o: any): Record<string, string> => {
  ...
  if (step === 3 && !/^\d{16}$/.test(String(o.card ?? '').replace(/\s/g, '')))
    e.card = 'A card number has 16 digits (any digits will do here).';
  return e;
};
...
export const POST: APIRoute = async ({ request }) => {
  ...
  const lines = Object.entries(order.qty ?? {})
    .filter(([, q]) => Number(q) > 0)
    .map(([k, q]) => `<li>${q} × ${k}</li>`)
    .join('');
  ...
  return sse(
    patchElements(
      `<div id="receipt"><h4>Order confirmed</h4><ul>${lines}</ul><p>Total <b>${total.toFixed(2)} EUR</b>, receipt sent to ${order.email}.</p></div>`,
    ),
