...
const things = ['the morph algorithm', 'a streaming endpoint', 'the checkout wizard', 'signal batching', 'the Astro SDK', 'a data table', 'the docs sidebar']
...
export const page = (p: number) => Array.from({ length: PAGE_SIZE }, (_, i) => p * PAGE_SIZE + i + 1).filter((n) => n <= TOTAL).map(post)
