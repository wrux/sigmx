...
The suite has four layers, each runnable on its own:
...
| `npm run test:browser` | what a simulated DOM cannot do: transitions and animations, observers, WebSocket, focus trapping, boost, the standalone script-tag build | Playwright drives `tests/browser/index.html` against the dev server in headless Chromium |
| `npm run test:sdks` | the Astro integration hooks and the Hono middleware | `node:test`, requests made with `app.request()` |

`npm test` builds, then runs the unit and DOM layers; `npm run test:all` adds the browser and SDK layers. `npm run test:coverage` prints Node's coverage report for the first two. The browser layer needs Chromium once: `npx playwright install chromium`.
...
Continuous integration runs every layer on each push and pull request (`.github/workflows/ci.yml`): lint, type-check, unit and DOM tests on Node 20 and 22, the SDK suites, the browser layer, the docs build and the example apps.
