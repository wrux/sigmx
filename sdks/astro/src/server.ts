...
export const patchElements = (html: string, o: PatchElementsOptions = {}): ServerEvent => {
  const lines: string[] = []
...
export const formatEvent = (e: ServerEvent): string =>
  [e.id !== undefined && `id: ${e.id}`, e.retry !== undefined && `retry: ${e.retry}`, `event: ${e.event}`, ...e.lines.map((l) => `data: ${l}`)]
    .filter(Boolean)
    .join('\n') + '\n\n'
...
export class SigmxStream {
  ...
  constructor(init: ResponseInit = {}) {
    ...
    this.response = new Response(body, { ...init, headers: { ...SSE_HEADERS, ...(init.headers as Record<string, string>) } })
  }
  get closed(): boolean {
    return !this.open
  }
...
export const sse = (...events: ServerEvent[]): Response => new Response(events.map(formatEvent).join(''), { headers: SSE_HEADERS })
...
export const html = (body: string, o: PatchElementsOptions & { status?: number } = {}): Response =>
  new Response(body, {
    status: o.status ?? 200,
    headers: {
...
export type SchemaIssue = { readonly message: string; readonly path?: ReadonlyArray<PropertyKey | { readonly key: PropertyKey }> }
export type SchemaResult<T> = { readonly value: T; readonly issues?: undefined } | { readonly issues: ReadonlyArray<SchemaIssue> }
...
export async function readSignals(request: Request, schema?: StandardSchema<any>): Promise<any> {
  ...
  if (result.issues) throw new SignalsError(`invalid signals: ${result.issues.map((i) => i.message).join('; ')}`, result.issues)
  return result.value
}
