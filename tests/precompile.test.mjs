...
import { rewriteSignals, compileBody, generateTable, extractExpressions } from '../dist/kernel/precompile.js'
import { precompiled, expressionKey } from '../dist/kernel/precompiled.js'
...
test('extractExpressions + generateTable + precompiled round trip', async () => {
  ...
  assert.deepEqual(items.map((i) => i.src), ['{ n: 1 }', '$n * 2', '$n++'])
