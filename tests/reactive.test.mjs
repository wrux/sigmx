...
import { signal, computed, effect, batch, untracked } from '../dist/kernel/reactive.js'
...
test('computed is lazy, cached, and does not cascade when its value is unchanged', () => {
  const n = signal(1)
  let computes = 0
  const even = computed(() => { computes++; return n.value % 2 === 0 })
...
test('dispose stops an effect and nested effects die with their parent run', () => {
  const a = signal(0), b = signal(0)
  let inner = 0
  const stop = effect(() => { a.value; effect(() => { b.value; inner++ }) })
