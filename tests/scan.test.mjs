...
import { builtinPlugins, customPluginMeta, selectPlugins, generatePluginsModule, mentions } from '../dist/kernel/scan.js'
...
test('selectPlugins: usage, implied dependencies, always, unused', () => {
  const sel = selectPlugins(['<div data-signals="{n:1}"><b data-text="$n"></b><button data-on:click="@get(\'/x\')"></button></div>'], { always: ['persist'] })
  ...
  assert.deepEqual(boosted.plugins.map((p) => p.export).sort(), ['applyElements', 'applyState', 'boost', 'httpGet', 'httpPost'].sort())
})
...
export const toast = handler({ name: 'toast', handle() {} })`
  ...
  const used = selectPlugins(['<p data-upper="$x"></p>', "stream.send({ event: 'sigmx-toast' })"], { custom: [upper, toast] })
  assert.deepEqual(used.plugins.map((p) => p.export), ['upper', 'toast'])
  const unused = selectPlugins(['<p data-text="$x"></p>'], { custom: [upper, toast] })
  assert.deepEqual(unused.unused.filter((n) => ['upper', 'toast'].includes(n)), ['upper', 'toast'])
