...
- `@sigmx/hono`: `serveClient({ path })` serves a bundle of your own with the same ETag and cache headers.
- Examples: the Express and Hono examples now build their client with `sigmxAuto()` from `sigmx/vite` (`npm run build`, also run before `dev` and `start`) and serve `public/sigmx.js`, 14 plugins at 9.3 KB brotli instead of the 11.8 KB every-plugin build; the Astro example sets `precompile: { fallback: false }`, leaving the runtime compiler out (8.4 KB brotli for its 13 plugins).
